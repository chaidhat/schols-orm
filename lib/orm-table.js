...
class DatabaseTable {
    ...
    async insertInto(entry) {
        // spinwait for lock
        let i = 0;
        if (ormSql.insertLock[this.tableKeyName] === undefined) {
            ormSql.insertLock[this.tableKeyName] = false;
        }
        while (ormSql.insertLock[this.tableKeyName] && i < 10000) {
            // wait 1 ms
            // if lock is not acquired in 10s (10,000 ms), then we force it
            await new Promise(resolve => setTimeout(resolve, 1));
            i++;
        }
        // take the lock
        ormSql.insertLock[this.tableKeyName] = true;
        if (i >= 10000) {
            const err = `orm warning: exceeded wait time for insert lock. Forced it open.`;
            console.log(err);
            ormHelper.logError(err);
        }
        
        // surround in try {} finally {} block to release lock once we're done
        let nextKey;
        try {
            if (entry === null) {
    ...
    async deleteFrom(where) {
        ...
        for (let i = 0; i < this.properties.length; i++) {
            ...
            if (ormHelper.isTokenPrimitive(propertyType.dataType)) {
                // Skip primitive types since they don't need special deletion handling
                continue;
            } else {
                if (propertyType.isArray) {
                    // insert a 1-M function
...
// CRUD
// get
export async function get${this.tableName} (token, ${this.tableKeyName}) {
    //TODO: auth
...
// create
export async function create${this.tableName} (
    token, 
    ${propertyNameListStr}
) {
    //TODO: auth
...
// change
export async function change${this.tableName} (
    token, 
    ${this.tableKeyName},
    ${propertyNameListStr}
) {
...
// remove
export async function remove${this.tableName} (token, ${this.tableKeyName}) {

// Import orm-sql.js later to avoid circular dependency
...
export async function popTable(table) {
...
function _checkTableNames() {
    for (let i = 0; i < databaseTables.length; i++) {
        ...
        if (match === null) {
            throw `orm fatal: table name ${databaseTables[i].tableName} must not contain any special characters. Use CamelCase for table names.`;
        }
        const match2 = databaseTables[i].tableKeyName.match(/^[a-zA-Z0-9]+$/);
        if (match2 === null) {
            throw `orm fatal: table key name ${databaseTables[i].tableKeyName} must not contain any special characters. Use pascalCase for table key values.`;
        }
...
async function _checkTableDbConsistency() {
    for (let i = 0; i < databaseTables.length; i++) {
        ...
        const resPromise = ormSql.sqlQuery(`SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = "${databaseTable.tableName}"`, true, true);
        ...
        if ("int" !== dataTypeMap[databaseTable.tableKeyName]) {
            console.log(`orm fatal: schema '${databaseTable.tableName}.${databaseTable.tableKeyName}' has type 'int' but in db it has type '${dataTypeMap[databaseTable.tableKeyName]}'`);
        }
...
async function _checkOpenTransactions() {
    const resPromise = ormSql.sqlQuery(`SELECT * FROM INFORMATION_SCHEMA.INNODB_TRX`, true, true);
    const res = await resPromise;
    if (res.length > 0) {
        throw `orm fatal: there is an uncommitted, open transaction. Either commit or rollback it.`;
    }
    return;
}
